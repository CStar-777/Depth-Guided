...
class MyTrainer(SimpleTrainer):
    """
    Like :class:`AMPTrainer`, but uses CopyPaste and GMM in the training loop.
    """
...
class SimpleTrainerSSL(TrainerBase):
    ...
    def update_ema_module(self, module, ema_module, ema_decay):
        ...
        for name, param in module_params.items():
            ema_module_params[name].sub_(
                (1.0 - ema_decay) * (ema_module_params[name] - param)
            )
        ...
        for name, buffer in module_buffers.items():
            if buffer.dtype == torch.float32:
                ema_module_buffers[name].sub_(
                    (1.0 - ema_decay) * (ema_module_buffers[name] - buffer)
                )
            else:
    ...
    def run_step(self):
        ...
        """
        if (
            self.model.module.do_ssl
            and self.model.module.iter % self.model.module.ssl_freq == 0
        ):
            data_unl = next(self._data_loader_unl_iter)
        else:
            data_unl = None
        ...
        if self.iter == self.model_teacher.module.burn_in:
            self.update_teacher_model(ema_decay=0.0)
...
class MyTrainerSSL(SimpleTrainerSSL):
    """
    copy by class AMPTrainerSSL, add the DepthAnything to correct the pseudo label
    """
    ...
    def __init__(
        ...
        self.depth_anything = DepthAnything(model_configs[encoder]).to(DEVICE)
        self.depth_anything.load_state_dict(
            torch.load(f"/home/xxxy/cx-project/GuidedDistillation/Depth-Anything/checkpoints/depth_anything_{encoder}14.pth")
        )
    ...
    def run_step(self):
        ...
        with torch.no_grad():
            teacher_preds = self.model_teacher(data_unl, return_preds=True)
            # teacher_preds["pred_logits"]/["pred_masks"]/["pred_aux_outputs"]
            # teacher_preds["pred_logits / pred_masks"][0].size() = [100, 81]/[100, 128, 128]/
            # teacher_preds["aux_outputs"][0]["pred_logits"]/["pred_masks"]
            # [4, 100, 81] / [4, 100, 128, 128]
            # self.model_teacher.module.save_images(self.iter, teacher_preds["masks"], [103.530, 116.280, 123.675], [1.0, 1.0, 1.0])
        teacher_pl = self.model_teacher.module.prepare_ssl_outputs(teacher_preds)
        # teacher_pl[0]:["label"]/["masks"][label_num, 128, 128] 
        # cx add:
        data_unl_copy = copy.deepcopy(data_unl)
        img_num = len(data_unl)
        for i in range(0, img_num):
            image = cv2.cvtColor(cv2.imread(data_unl[i]["file_name"]), cv2.COLOR_BGR2RGB) / 255.0
            # # 下面可视化img和mask
            # img = cv2.imread(data_unl[i]["file_name"])
            # # cv2.imwrite("/home/xxxy/cx-project/GuidedDistillation/out/infer_img/test.jpg", img)
            # mask = teacher_pl[i]["masks"].int()
            # mask = mask.detach().cpu().numpy()
            # print("imsave mask image id:",data_unl[i]["file_name"])
            # # cv2.imwrite('/home/xxxy/cx-project/GuidedDistillation/out/infer_img/output.jpg', mask)
            # b,g,r = cv2.split(img)
            # img2 = cv2.merge((r,g,b))
            # plt.imsave("/home/xxxy/cx-project/GuidedDistillation/out/infer_img/test.jpg", img2)
            # num = len(mask)
            # name = data_unl[i]["file_name"]
            # import os
            # name = os.path.basename(name)
            # name = name.split('.')[0] 
            # for j in range(0,num-1):
            #     plt.imsave(f'/home/xxxy/cx-project/GuidedDistillation/out/infer_img/mask_{name}_{j}.jpg', mask[j], cmap='gray')
            # question:mask是128*128的crop！不是全图，也不知道把原图缩放到多少再crop的，还找不到坐标
            # plt.subplot(121),plt.imshow(img2),plt.title('IMG')  #显示彩色图
            # plt.subplot(122),plt.imshow(mask),plt.title('IMG1') #显示灰度图时，要设置‘,cmap='gray'’  读取的图片也应是灰度图
            # plt.show()
            ...
            with torch.no_grad():
                depth, depth_features = self.depth_anything(image) #torch.Size([1, 518, 686])
            depth = F.interpolate(depth[None], (h, w), mode="bilinear", align_corners=False)[0, 0]
            depth = (depth - depth.min()) / (depth.max() - depth.min()) * 255.0

            depth = depth.cpu().numpy().astype(np.uint8)
            depth = cv2.applyColorMap(depth, cv2.COLORMAP_INFERNO)
            data_unl_copy[i]["image"] = depth
            
        with torch.no_grad():
            depth_img_preds = self.model_teacher(data_unl_copy, return_preds=True)
        depth_img_pl = self.model_teacher.module.prepare_ssl_outputs(depth_img_preds)
        
        data_ssl = {"data": data_unl, "pseudo_label": teacher_pl, "depth_pl": depth_img_pl, "depth_features": depth_features}
        # data_ssl = {"data": data_unl, "pseudo_label": teacher_pl}
        del teacher_preds
        del depth_img_preds
