...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs, branch='supervised', return_preds=False):
        ...
        if branch == 'supervised': #not do_ssl or self.iter < self.burn_in:
            ...
            # print("imwrite image id:",batched_inputs['data'][0]["file_name"])
            # name = batched_inputs['data'][0]["file_name"]
            # import os
            # name = os.path.basename(name)
            # name = name.split('.')[0] 
            # # cx add
            # image = images_unl[0]*self.pixel_std+self.pixel_mean
            # image = image.detach().cpu().numpy().transpose(1,2,0)
            # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            # cv2.imwrite(f'/home/xxxy/cx-project/GuidedDistillation/out/infer_img/test_{name}.jpg', image)
            
            # Student predictions.
            # First perturbation stream.
            features = self.backbone(images_unl)
            # cx add:学生提取的特征与深度特征融合
            depth_features = batched_inputs['depth_features']
            outputs = self.sem_seg_head(features)
            ...
            # cx add
            losses_depth = self.ssl_criterion(outputs, batched_inputs['depth_pl'])
            for p in list(losses_depth.keys()):
                if p in self.ssl_criterion.weight_dict:
                    losses_depth[p] *= self.ssl_criterion.weight_dict[p]
                else:
                    # remove this loss if not specified in `weight_dict`
                    losses_depth.pop(p)
            ...
            for p, q in losses_depth.items():
                losses_ssl[p+ "_ssl_d"] = 1.0*q
